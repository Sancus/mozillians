...
class UserProfileTests(TestCase):
    ...
    def test_accounts_access(self):
        user = UserFactory.create()
        user.userprofile.externalaccount_set.create(type='SUMO', identifier='test')
    ...
    def test_accounts_public_mozillians(self):
        ...
        profile.set_instance_privacy_level(PUBLIC)
        profile.externalaccount_set.create(type='SUMO', identifier='test', privacy=MOZILLIANS)
    ...
    def test_websites(self):
        ...
        profile.set_instance_privacy_level(MOZILLIANS)
        profile.externalaccount_set.create(type='SUMO', identifier='test', privacy=MOZILLIANS)
        profile.externalaccount_set.create(type='WEBSITE', identifier='http://google.com', privacy=MOZILLIANS)
        eq_(profile.accounts.count(), 1)
...
class ExternalAccountTests(TestCase):
    def test_get_url(self):
        profile = UserFactory.create().userprofile
        account = profile.externalaccount_set.create(type='MDN', identifier='sammy')
        ok_('sammy' in account.get_identifier_url())

    def test_get_url_unicode(self):
        profile = UserFactory.create().userprofile
        account = profile.externalaccount_set.create(type='MDN', identifier=u'sammyウ')
